jQuery.msgbox= {
					...
					"aoColumnDefs" : [
						{
							...
							'fnRender' : function(oObj, sVal) {
								return"  <button class=\"btn2 btn-info\" onclick=\"$.msgbox.deleteUser("+oObj.aData.id+")\"><i class=\"icon-trash\"></i> 删除</button>";
							}
						},
					 {
						'aTargets' : [ '_all' ],
		...
		deleteUser :function(id){
			bootbox.confirm( "是否确认删除？", function (result) {
	            if(result){
	            	$.ajax({
	        			type : "get",
	        			url : $.ace.getContextPath() + "/admin/msgbox/delete/"+id,
	        			dataType : "json",
	        			success : function(json) {
	        				if(json.state=='success'){
	        					noty({"text":""+ json.msg +"","layout":"top","type":"success","timeout":"2000"});
	        					$.msgbox.initSearchDataTable();
	        				}else{
		...
		showEdit: function (id){
			...
			$.ajax({
    			type : "get",
    			url : $.ace.getContextPath() + "/admin/msgbox/get/"+id,
    			dataType : "json",
    			success : function(json) {
    				if(json.state=='success'){
